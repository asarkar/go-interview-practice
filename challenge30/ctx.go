...
func (cm *simpleContextManager) CreateCancellableContext(
	parent context.Context,
) (context.Context, context.CancelFunc) {
...
func (cm *simpleContextManager) CreateTimeoutContext(
	parent context.Context,
	timeout time.Duration,
) (context.Context, context.CancelFunc) {
...
func (cm *simpleContextManager) ExecuteWithContext(ctx context.Context, task func() error) error {
	...
	go func() {
		...
		// Close after sending/abandoning.
		close(errCh)
	}()
...
func (cm *simpleContextManager) WaitForCompletion(
	ctx context.Context,
...
func SimulateWork(ctx context.Context, workDuration time.Duration, description string) error {
	...
	fmt.Printf("Starting work: %s (duration: %v)\n", description, workDuration)
...
func ProcessItems(ctx context.Context, items []string) ([]string, error) {
	...
	for i := range items {
		...
		default:
			processed = append(processed, "processed_item"+strconv.Itoa(i+1))
			if err := SimulateWork(
				ctx,
				100*time.Millisecond,
				"Processing item: "+items[i],
			); err != nil {
				return processed, err // respect cancellation mid-item
			}
		}
	}
