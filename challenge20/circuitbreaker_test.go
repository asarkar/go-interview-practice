...
// This test was updated from its original form. In the half-open state, even if one request fails,
// the CB moves to the open state. If `n` number of requests all succeed, the CB transitions to
// closed state. At no time, the CB will receive more than `n` requests in the half-open state.
func TestHalfOpenToClosedTransition(t *testing.T) {
	...
	if cb.GetState() != StateClosed {
		t.Errorf(
			"Expected state to be Closed after successful half-open operation, got %v",
			cb.GetState(),
		)
	}
}
...
func TestHalfOpenToOpenTransition(t *testing.T) {
	...
	if cb.GetState() != StateOpen {
		t.Errorf(
			"Expected state to be Open after failed half-open operation, got %v",
			cb.GetState(),
		)
	}
}

// In the half-open state, even if one request fails, the CB moves to the open state.
// If `n` number of requests all succeed, the CB transitions to closed state.
// At no time, the CB will receive more than `n` requests in the half-open state.
