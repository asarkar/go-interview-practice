	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.shouldError {
				if err == nil {
					t.Errorf(
						"Expected an error for width=%.2f, height=%.2f, but got none",
						tt.width,
						tt.height,
					)
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.shouldError {
				if err == nil {
					t.Errorf(
						"Expected an error for sides (%.2f, %.2f, %.2f), but got none",
						tt.a,
						tt.b,
						tt.c,
					)
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, part := range tt.expectedParts {
				if !strings.Contains(strings.ToLower(str), strings.ToLower(part)) {
					t.Errorf(
						"Expected string representation to contain '%s', but got: %s",
						part,
						str,
					)
				}
...
func TestShapeCalculatorLargestShape(t *testing.T) {
	...
	if largest != rect {
		t.Errorf(
			"Expected largest shape to be Rectangle, got %T with area %.2f",
			largest,
			largest.Area(),
		)
	}
