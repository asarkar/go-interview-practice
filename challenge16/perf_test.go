	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !slicesEqual(slowResult, optimizedResult) {
				t.Errorf(
					"OptimizedSort gave different results than SlowSort. Got %v, expected %v",
					optimizedResult,
					slowResult,
				)
			}
...
func TestInefficientStringBuilder(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if result != expected {
				t.Errorf(
					"InefficientStringBuilder result incorrect. Got %q, expected %q",
					result,
					expected,
				)
			}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if optimizedResult != inefficientResult {
				t.Errorf(
					"OptimizedStringBuilder gave different results than InefficientStringBuilder. Got %q, expected %q",
					optimizedResult,
					inefficientResult,
				)
			}
...
func TestOptimizedCalculation(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		if optimizedResult != expensiveResult {
			t.Errorf(
				"OptimizedCalculation(%d) = %d, expected %d",
				tc.n,
				optimizedResult,
				expensiveResult,
			)
