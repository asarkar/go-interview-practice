...
import (
	...
	// The github.com/mattn/go-sqlite3 driver registers itself as a database/sql driver in its
	// init() function:
	//
	// 	func init() {
	...
	// 	}
	//
	// By doing a blank import, the init() function runs and the driver registers itself.
	// Without the import, sql.Open("sqlite3", "store.db") would fail:
...
func TestMain(m *testing.M) {
	// Setup
	db, err := InitDB("test_inventory.db")
	if err != nil {
		log.Fatalf("could not open test database: %v", err)
	}
	...
	err = testDB.Close()
	if err != nil {
		log.Fatalf("could not close test database: %v", err)
	}
